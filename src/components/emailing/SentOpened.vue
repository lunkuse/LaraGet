<template>
    ...
    <!-- <div class="col-12 box inset-0 flex flex-col p-4 text-gray-900 bg-gray-100 dark:bg-gray-900 dark:text-white top-6"> -->
      ...
      <div class="flex items-center text-lg intro-y  ">
        ...
        <div class="flex" >
          <Avatars v-if="fullName" v-bind:initials="fullName"  class="rounded-full"/>
           <div class="leading-none text-gray-800 col-span-4 space-y-1 dark:text-gray-200 pl-5">
          <div >
            {{fullName}}
            <span class="ml-2 text-gray-500 text-xxs dark:text-gray-400"> 
              {{ moment(messageInfo?.created_at ).fromNow()}}
              </span>
          </div>
      ...
      <div class="flex flex-col justify-between flex-1  overflow-auto ml-10 pl-3">
        <div>
          ...
          <p  class="mt-3 mb-3 dark:text-white"  v-html="myMessage"></p>
            <hr>
                          <div class="read-content-attachment"  >
                              <div class="container mx-auto justify-center mt-4" style="display: flex;
       align-items: center; text-align: center; text-gray-600">
  ...
  <div class="col-12 flex flex-col p-4 text-gray-900 top-6" >
       ...
       <span
        ...
        >
          ...
          <div class="row attachment" v-for="reply in messageReplies" :key="reply.id">
            ...
            <div class="mt-6">
              <div class="flex" >
                <!-- <Avatars v-bind:initials="viewMgs.from_fname + viewMgs.from_lname" /> -->
                <Avatars v-bind:initials="reply.from_fname + reply.from_lname" class="rounded-full" />
                <div class="leading-none text-gray-800 col-span-4 space-y-1 dark:text-gray-200 pl-5">
                  <div v-if="viewMgs">
                   <span v-if="reply.from_user_id === myId"> {{$t("translation.me_text") }} </span>
                   <span v-else> {{reply.from_fname}} {{reply.from_lname}} </span>
                   <!--  {{reply.from_user_id}} {{ myId}} -->
                    <span
                      class="ml-2 text-gray-500 text-xxs dark:text-gray-400"
              ...
              <div>
                <!-- <p>Hello,</p> -->
                <!-- <p v-if="viewMgs" class="mt-3" v-html="viewMgs.message"></p> -->
  ...
  export default {
    ...
    methods: {
      ...
      async printMessage() {
        const defaultLang = this.storedLang ?? "nl";
      const lang =defaultLang;
       this.loadingexport=true
        const token = localStorage.getItem('token')
        const config = {
          url: 'v2/mails/download-single-outbox-message-pdf',
          method: 'get',
          responseType: 'blob',
          params: {
      ...
      //  messageSubject
        ...
        axios.post('v2/mails/read_sent', data, {
            headers: {
              Authorization: 'Bearer ' + token,

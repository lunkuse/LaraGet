...
export default function useUtilities() {
  ...
  const markDecay = () => {
    // console.log(treatmentBlob.value);
    // Pathology Layer
    ...
    if (typeof restorationTreatment !== 'undefined') {
      ...
      if (typeof restorationArray !== 'undefined'&& !treatmentBlob.value.missing.includes(activeTooth.value?.toString())) {
    ...
    if (typeof pathologyDiagnosis !== 'undefined') {
      let decayArray = pathologyDiagnosis['decay'];
      if (typeof decayArray !== 'undefined'&& !treatmentBlob.value.missing.includes(activeTooth.value?.toString())) {
        tempPathologyAreas = decayArray;

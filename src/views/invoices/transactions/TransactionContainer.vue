      ...
      >
        <div class="w-auto gap-2 text-sm mt-2 content-center flex items-center">
          ...
          >
            ...
            {{ $t('translation.transactions_text') }}
          </div>
          <div
            class="mx-auto text-slate-500"
        ...
        >
          ...
          >
            ...
            >
              ...
              <input
                class="w-full block p-1 outline-0 appearance-none placeholder-gray-400 text-gray-700 z-10 bg-gray-100 dark:bg-dark-1 cursor-pointer"
                  ...
                  <div class="border-t border-gray-300 items-center p-1 w-32">
                    <Button
                      ...
                      @click="
                        printObject(
                          'pdf',
                          translations?.transactions_text ?? 'Transactionss'
                        )
                      "
                      class="border-t bold border-0 cursor-pointer text-black bg-white w-full hover:bg-gray-100 text-sm h-7 !text-gray-800 !bg-white !text-sm !capitalize"
                    ></Button>
                  ...
                  <div class="border-t border-gray-300 items-center p-1 w-32">
                    <Button
                      ...
                      @click="
                        printObject(
                          'xlsx',
                          translations?.transactions_text ?? 'Transactions',
                          '/patients/invoices/download-transaction-excel'
                        )
                      "
                      class="border-t bold border-0 cursor-pointer text-black bg-white w-full hover:bg-gray-100 text-sm h-7 !text-gray-800 !bg-white !text-sm !capitalize"
                    ></Button>
                    ...
                    <Button
                      ...
                      @click="
                        printObject(
                          'csv',
                          translations?.transactions_text ?? 'Transactions',
                          '/patients/invoices/download-transaction-csv'
                        )
                      "
...
export default {
  ...
  data() {
    return {
      ...
      filterOptions: [
        {
          name: this.t('translation.open_text'),
          value: 'open'
        },
        {
      ...
      pagination: {
        last_page_url: null,
        next_page_url: 'patients/invoices/transactions/invoice'
      },
  ...
  methods: {
          ...
          try {
            axios
              ...
              .then((response) => {
                ...
                console.log('new response', this.invoices)
    ...
    async exportDataTable() {
      console.log('length', this.checkedInvoiceListMain.length)
      if (this.checkedInvoiceListMain.length > 0) {
        let invoices = []
    ...
    async printObject(
      type,
      filename = this.t('translation.transactions_text'),
      url = '/patients/invoices/download-transaction-pdf'
    ) {
      try {
        ...
        const config = {
          ...
          params: {
            transactions: JSON.stringify(invoice),
    ...
    async printData() {
      const transaction = await this.exportDataTable()
      const token = localStorage.getItem('token')
      const config = {
        url: '/patients//invoices/download-transaction-pdf',
        method: 'get',
...
this.hideFilterDates();
    ...
    selectOption(option) {
      ...
      if (option == 'open') {
        this.filter_option = this.t('translation.open_text')
  ...
  computed: {
    async paginatedInvoices() {
      ...
      // status filterStatus
        ...
        this.fetchAll({ status: this.filterStatus, invoiceId: this.invoiceID })
      }
      // this.past_appointments
      else if (!!this.past_appointments) {
        this.fetchAll({
          past_appointments: this.past_appointments,
          invoiceId: this.invoiceID
        })
      }
      ...
      // doctor filter
        ...
        this.fetchAll({
          doctors: this.selected_doctorsIDs,
          invoiceId: this.invoiceID
        })
      }
      ...
      // year filter
        ...
        this.fetchAll({ year: this.year_range, invoiceId: this.invoiceID })
      }
      ...
      // month range
        ...
        this.fetchAll({ invoiceId: this.invoiceID })
      }
    },
  ...
  watch: {
    ...
    filter_option(newsearch_term, oldfilteroption) {
      if (!!newsearch_term) {
        ...
        if (newsearch_term === this.t('translation.open_text')) {

<template>
    ...
    >
      ...
      >
        <div class="w-auto gap-2 text-sm mt-2 content-center flex items-center">
          ...
          >
            {{ $t("translation.showing_text") }} {{ lengthFrom }}
            {{ $t("translation.to_text") }} {{ lengthTo }}
            {{ $t("translation.of_text") }} {{ lengthTotal }}
            {{ $t("translation.invoices_text") }}
          </div>
          <div
            class="mx-auto text-slate-500"
            v-if="checkedInvoiceListMain.length > 0"
          >
            {{ $t("translation.selected_text") }}
            {{ checkedInvoiceListMain.length }}
            {{ $t("translation.invoices_text") }}
            <span
        ...
        >
          ...
          >
            ...
            >
              ...
              <input
                class="border-gray-100 form-control w-full h-full block outline-0 appearance-none placeholder-gray-400 text-gray-700 bg-gray-100 dark:bg-dark-1 cursor-pointer focus:ring-0 focus:border-theme-36"
            ...
            >
              <div class="w-full flex items-center justify-between">
                <input
                  type="text"
                  class="w-full p-2 block bg-gray-100 dark:bg-dark-1 outline-0 appearance-none border-0 placeholder-gray-500 text-gray-700 z-10 cursor-pointer"
                ...
                >
                  ...
                  >
                    <input
                      ...
                      class="w-full block p-1 border-none capitalize bg-gray-100 dark:bg-dark-1 placeholder:text-slate-500 appearance-none text-slate-700 z-50 cursor-pointer"
            ...
            >
              <Button
                id="showfilterss"
                :label="`${$t('translation.filter_text')}`"
                ...
                >
                  ...
                  <div class="border-t border-gray-300 items-center p-1 w-32">
                    <Button
                      ...
                      @click="
                        printObject('pdf', t('translation.invoices_text'))
                      "
                      class="border-t bold border-0 cursor-pointer text-black bg-white w-full hover:bg-gray-100 text-sm h-7 !text-gray-800 !bg-white !text-sm !capitalize"
                    ></Button>
                  </div>
                    ...
                    <Button
                      ...
                      @click="
                        printObject(
                          'xlsx',
                          t('translation.invoices_text'),
                          '/patients/invoices/download-excel'
                        )
                      "
                      class="border-t bold border-0 cursor-pointer text-black bg-white w-full hover:bg-gray-100 text-sm h-7 !text-gray-800 !bg-white !text-sm !capitalize"
                    ></Button>
                    ...
                    <Button
                      ...
                      @click="
                        printObject(
                          'csv',
                          t('translation.invoices_text'),
                          '/patients/invoices/download-csv'
                        )
                      "
  ...
  {{$t('translation.save_text')}} -->
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <p>{{ translations?.exporting_text ?? "exporting" }}..</p>
                  </div>
                </div>
              </div>
        ...
        <InvoiceList
          ...
          @refreshInvoices="fetchMoreData"
          @checkedInvoiceList="checkedInvoiceList"
        ></InvoiceList>
      </div>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      filterOptions: [
        ...
        {
          name: this.t("translation.paid_text"),
          value: "1",
        },
      ],
      ...
      list: [],
      checkedInvoiceListMain: [],
      counter: 1,
      paginatedData: [],
      pagination: {
        last_page_url: null,
        next_page_url: "patients/invoices/all",
      },
  ...
  methods: {
    ...
    UncheckAllSelected() {
      ...
      if (this.checkedInvoiceListMain.length > 0) {
        this.checkedInvoiceListMain.length = [];
        this.isUnchenkallstatus = true;
      }

      console.log("isUnchenkallstatus", this.isUnchenkallstatus);
    },
    checkedInvoiceList(checkedInvoices) {
      this.checkedInvoiceListMain = checkedInvoices;
    },
  ...
  async  fetchAll(data) {
      ...
      if (
        ...
        if (this.pagination.next_page_url !== null) {
          ...
          await this.$store.dispatch('allinvoices/FetchInvoices', { url, data,finalToken });
          this.isLoading = false;
           const myinvoices =  this.$store?.state?.allinvoices?.patientInvoices;
          //  console.log('myinvoices', myinvoices)
          this.invoices = this.invoices.concat(
  myinvoices?.data
);
let currentPage = myinvoices?.current_page;
this.last_page = myinvoices?.last_page_url;
let next_page_url = myinvoices?.next_page_url;
this.links = myinvoices?.links;
this.pagination = {
  last_page_url: myinvoices?.last_page_url,
  next_page_url: myinvoices?.next_page_url,
};
// this.lengthFrom=response.data.payload.from
this.lengthTo = myinvoices?.to;
this.lengthTotal = myinvoices?.total;
          // try {
          //   InvoiceService.fetchInvoices(url, data, this.ourRequest.token)
          //     .then((response) => {
          ...
          //       this.invoices = this.invoices.concat(response?.data?.data);
    ...
    async exportDataTable() {
      if (this.checkedInvoiceListMain.length > 0) {
        let invoices = [];
        invoices = this.checkedInvoiceListMain;
        return invoices;
      } else {
        let invoices = [];
    ...
    async printObject(
      type,
      filename = translations?.invoices_text ?? "Invoices",
      url = "/patients/invoices/download-pdf"
    ) {
      try {
        const invoice = await this.exportDataTable();
        const token = localStorage.getItem("token");
        const config = {
          url: url,
          method: "get",
    ...
    clearall() {
      ...
      this.secondDate = "";
      this.year = null;
      (this.year_range = null), (this.status = null);
      this.filterStatus = null;
      this.filter_option = null;
      this.isFilterOptionsForExports = false;
      this.doctorsExpanded = false;
      ...
      this.filterStatus = null;
      this.checkedInvoiceListMain.length = 0;
      this.doctorName = null;
      this.date_range = null;
      this.month_range = null;
      this.pagination = {
    ...
    selectOption(option) {
      console.log("filter_option option option", option);
      if (option == "0") {
        this.filter_option = this.t("translation.pending_text");
        console.log("filter_option1 filter_option", this.filter_option);
      } else if (option == "1") {
        this.filter_option = this.t("translation.paid_text");
        console.log("filter_option2 filter_option", this.filter_option);
      } else {
        this.filter_option = option;
        console.log("filter_option3 filter_option", this.filter_option);
      }
    ...
    checkedDoc(event, doctor) {
      if (event.target.checked) {
        ...
        const appointments = [...this.invoices];
  ...
  computed: {
    async paginatedInvoices() {
      ...
      // status
        ...
        this.fetchAll({ status: parseInt(this.filterStatus) });
      }
    ...
    filteredInvoices() {
      // let allAppointments = this.invoices;
      let allAppointments = [...this.invoices];
  ...
  watch: {
    ...
    filter_option(newsearch_term, oldfilteroption) {
      if (!!newsearch_term) {
        ...
        console.log("selected stat bbbb all", newsearch_term);
        if (newsearch_term === this.t("translation.pending_text")) {
          console.log("selected stat 2 pending", newsearch_term);
          this.filterStatus = "0";
          this.invoices = [];
          this.paginatedData = [];
          this.year = null;
          ...
          console.log("this.invoices 2", this.invoices);
          this.UncheckAllSelected();
          this.paginatedInvoices;
        } else if (newsearch_term === this.t("translation.paid_text")) {
          console.log("selected stat 2 paid");

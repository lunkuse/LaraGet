<template>
  <div class="tab-pane active" role="tabpanel" id="_7gmksu1jl" aria-labelledby="_7gmksu1jl-tab">
    <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
      <h2 class="text-lg font-medium mr-auto">Treatment Details</h2>
      <!-- loader text -->
    ...
    <div class="box p-5 mt-5" v-if="!loading">
      <div class="flex-col items-center border-b border-slate-200 dark:border-darkmode-400 pb-5">
        <!-- date -->
        <div
          class="pl-4 w-full bg-indigo flex flex items-center border-b border-slate-200 dark:border-darkmode-400 py-5"
        >
          <div class="w-full lg:w-2/3 bg-blue h-8 font-bold">Treatment Date</div>
        ...
        >
          ...
          <div
            class="w-full lg:w-1/3 bg-red h-8"
            v-if="(client_appointments.doctors)"
          >{{client_appointments.doctors[0]['first_name']}} {{client_appointments.doctors[0]['last_name']}}</div>
        </div>
        ...
        >
          ...
          <div
            class="w-full lg:w-1/3 bg-red h-8"
            v-if="(client_appointments.slots)"
          >{{client_appointments.treatment_type ? client_appointments.treatment_type.title: client_appointments.appointment_type.title}}</div>
        </div>
        ...
        >
          ...
          <div v-if="client_appointments.created_at" class="w-full lg:w-1/3 bg-red h-8">
            ...
            <div
              v-else
            >{{moment(client_appointments.created_at, "YYYY-MM-DD").format("DD MMMM YYYY")}}</div>
          </div>
        </div>
        <!-- comment-->
    ...
    getAppointments() {
      ...
      axios
        ...
        .then(res => {
          ...
          console.log('details', this.client_appointments)
